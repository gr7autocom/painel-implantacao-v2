...
export function CategoriasTab() {
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              ...
              items.filter((c) =>
                filtroAtivo === 'todos' ? true : filtroAtivo === 'ativo' ? c.ativo : !c.ativo
              ).map((c) => (
      ...
      >
        <form id="categoria-form" onSubmit={save} className="space-y-4">
          <div>
      ...
      >
        <p className="text-sm text-gray-600">
          Excluir a categoria <strong>{confirmDelete?.nome}</strong>? Esta ação não pode ser desfeita.
        </p>
      </Modal>
    </div>
  )
