...
export function ImplantacaoTab() {
  const [items, setItems] = useState<EtapaImplantacao[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [modalOpen, setModalOpen] = useState(false)
  const [editing, setEditing] = useState<EtapaImplantacao | null>(null)
  const [form, setForm] = useState<FormState>(emptyForm)
  const [saving, setSaving] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<EtapaImplantacao | null>(null)
  ...
  async function load() {
    ...
    else setItems((data ?? []) as EtapaImplantacao[])
    setLoading(false)
  }
  ...
  function openEdit(item: EtapaImplantacao) {
    setEditing(item)
    setForm({
      nome: item.nome,
      ordem: item.ordem,
  ...
  async function save(e: React.FormEvent) {
    ...
    const result = editing
      ? await supabase.from('etapas_implantacao').update(payload).eq('id', editing.id)
      : await supabase.from('etapas_implantacao').insert(payload)
    setSaving(false)
    if (result.error) {
      setError(
        result.error.code === '42501'
          ? 'Você não tem permissão para editar catálogos.'
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Etapas de Implantação</h2>
          <p className="text-xs text-gray-500 mt-0.5">
            Estágios do projeto (A fazer → Inaugurado). Diferente das etapas de tarefa.
          </p>
        </div>
      ...
      <Modal
        ...
        title={editing ? 'Editar Etapa de Implantação' : 'Nova Etapa de Implantação'}
        footer={
          <>
            <Button variant="secondary" type="button" onClick={() => setModalOpen(false)}>
              Cancelar
            </Button>
            <Button type="submit" form="etapa-impl-form" disabled={saving}>
              {saving ? 'Salvando...' : 'Salvar'}
            </Button>
          </>
        }
      >
        <form id="etapa-impl-form" onSubmit={save} className="space-y-4">
          <div>
