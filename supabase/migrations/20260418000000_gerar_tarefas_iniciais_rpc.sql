-- RPC SECURITY DEFINER que gera as tarefas iniciais de um cliente.
-- Substitui o INSERT client-side (que estava falhando silenciosamente).
--
-- Características:
--   - Idempotente: se o cliente já tiver qualquer tarefa, retorna 0 sem inserir nada
--     (protege contra duplicação acidental em re-submits)
--   - Valida que o chamador tem capacidade `cliente.criar`
--   - Gera tudo dentro de uma única transação — tudo ou nada
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_existentes FROM tarefas WHERE cliente_id = p_cliente_id;
  ...
  FOR v_i IN 1..v_cliente.servidores_qtd LOOP
    INSERT INTO tarefas (titulo, categoria_id, classificacao_id, etapa_id, prioridade_id, cliente_id, criado_por_id, responsavel_id)
    VALUES (
      'Instalação de Servidor (' || v_i || '/' || v_cliente.servidores_qtd || ')',
      v_categoria_id, v_classif_sis_id, v_etapa_id, v_prio_id,
      p_cliente_id, v_criado_por_id, NULL
    );
    v_count := v_count + 1;
  END LOOP;
