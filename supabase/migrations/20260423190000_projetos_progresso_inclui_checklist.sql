-- Fix: view projetos_progresso agora considera itens de checklist no cálculo.
-- Antes: pct/total/concluidos contavam só tarefas; o CTE checklist_base era computado
--        mas nunca usado no SELECT final (bug de implementação).
-- Agora: cada tarefa vale 1 unidade + cada item de checklist vale 1 unidade adicional.
--        Tarefa cancelada e items dela ficam fora.
--        status_atividade só vira 'concluido' quando tarefas concluídas E items ticados.
  ...
  CASE
    ...
    WHEN ei.nome ILIKE '%inaug%'
         AND (COALESCE(b.concluidos, 0) + COALESCE(c.concluidos_items, 0))
             = (COALESCE(b.total, 0) + COALESCE(c.total_items, 0)) THEN 'concluido'
    ...
    WHEN ei.nome ILIKE '%conclu%'
         AND (COALESCE(b.concluidos, 0) + COALESCE(c.concluidos_items, 0))
             = (COALESCE(b.total, 0) + COALESCE(c.total_items, 0)) THEN 'aguardando_inauguracao'
