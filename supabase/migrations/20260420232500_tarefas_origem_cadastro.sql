-- Rastreabilidade: tarefas geradas automaticamente pelo cadastro do cliente
-- origem_cadastro=true → gerada pela RPC; não pode ser excluída diretamente
-- (deve-se editar o cliente e remover o item para cancelar a tarefa)

-- ──────────────────────────────────────────────────────────────
-- 1. Coluna na tabela tarefas
-- ──────────────────────────────────────────────────────────────
ALTER TABLE public.tarefas
  ADD COLUMN IF NOT EXISTS origem_cadastro BOOLEAN NOT NULL DEFAULT FALSE;

-- ──────────────────────────────────────────────────────────────
-- 2. Backfill: marca tarefas geradas pela RPC existentes
-- ──────────────────────────────────────────────────────────────
UPDATE public.tarefas
SET origem_cadastro = TRUE
WHERE de_projeto = TRUE
  AND (
    titulo LIKE 'Instalação de Servidor%'
    OR titulo LIKE 'Instalação de Retaguarda%'
    OR titulo LIKE 'Instalação de Caixa/PDV%'
    OR titulo LIKE 'Instalação módulo %'
  );

-- ──────────────────────────────────────────────────────────────
-- 3. Atualiza gerar_tarefas_iniciais_cliente para setar origem_cadastro
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- 4. RPC sincronizar_tarefas_cliente
--    Calcula delta ao editar cliente: cria novas, cancela removidas
-- ──────────────────────────────────────────────────────────────
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE projeto_id = v_projeto_id
      ...
      AND etapa_id IS DISTINCT FROM v_etapa_cancelado
  LOOP
    -- Extrai chave: remove prefixo "Instalação módulo ", substitui espaços por _
    v_mod := replace(
