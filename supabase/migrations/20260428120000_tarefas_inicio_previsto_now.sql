-- Todas as tarefas geradas automaticamente (por RPC) passam a ter
-- inicio_previsto = NOW() no momento da criação, seguindo o mesmo
-- comportamento do TarefaModal que já preenche a data atual no form.
-- Afeta: gerar_tarefas_iniciais_cliente e sincronizar_tarefas_cliente.
...
BEGIN
  ...
  VALUES (p_cliente_id, v_nome_final, v_cliente.etapa_implantacao_id)
...
BEGIN
  ...
  WHERE origem_cadastro = TRUE
    ...
    AND (projeto_id = v_projeto_id OR (cliente_id = p_cliente_id AND de_projeto = FALSE));
    ...
    FOR v_i IN (v_existing + 1)..v_cliente.servidores_qtd LOOP
      IF v_projeto_concluido THEN
        INSERT INTO tarefas (titulo, categoria_id, classificacao_id, etapa_id, prioridade_id,
                             ...
                             inicio_previsto)
        VALUES ('Instalação de Servidor (' || v_i || '/' || v_cliente.servidores_qtd || ')',
                COALESCE(v_cat_contrat_id, v_categoria_id), NULL,
                v_etapa_pendente, v_prio_id,
                p_cliente_id, NULL, FALSE, TRUE, v_criado_por_id, NOW());
      ELSE
        INSERT INTO tarefas (titulo, categoria_id, classificacao_id, etapa_id, prioridade_id,
                             cliente_id, projeto_id, de_projeto, origem_cadastro, criado_por_id,
                             inicio_previsto)
        VALUES ('Instalação de Servidor (' || v_i || '/' || v_cliente.servidores_qtd || ')',
                ...
                p_cliente_id, v_projeto_id, TRUE, TRUE, v_criado_por_id, NOW());
      END IF;
